...
class UtilisateurAmenagementEnCoursFilterHelper implements ResetInterface
{
    ...
    {
        return array_map(fn($val) => match (true) {
            is_numeric($val) => $val,
    ...
    public function ajouterJointuresCategories(
        QueryBuilder $queryBuilder,
        QueryNameGeneratorInterface $queryNameGenerator,
        array|string $categorieIds,
    ): string {
        if (!array_key_exists('categorie', $this->joins)) {
            ...
            $queryBuilder
                ->join(sprintf('%s.categorie', $typeAlias), $categorieAlias)
    ...
    private function ajouterJointuresAmenagement(
        QueryBuilder $queryBuilder,
        QueryNameGeneratorInterface $queryNameGenerator,
    ): string {
        if (!array_key_exists('amenagements', $this->joins)) {
            ...
            $queryBuilder
                ->join(sprintf('%s.beneficiaires', $utilisateurAlias), $beneficiairesAlias)
    ...
    public function ajouterJointuresType(
        QueryBuilder $queryBuilder,
        QueryNameGeneratorInterface $queryNameGenerator,
    ...
    public function ajouterJointuresDomaine(
        QueryBuilder $queryBuilder,
        QueryNameGeneratorInterface $queryNameGenerator,
        $value,
        $property,
        ...
        $queryBuilder->andWhere(sprintf('%s.%s = :%s', $typeAlias, $property, $domaineParam))->setParameter(
            $domaineParam,
            $value == 'true',
        );
    }
