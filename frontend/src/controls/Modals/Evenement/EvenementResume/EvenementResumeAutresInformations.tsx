        ...
        <Descriptions.Item label="Gestionnaire en charge de l'organisation">
          <List size="small">
            <ul className="list-nostyle">
              {[props.evenement?.utilisateurCreation, props.evenement?.utilisateurModification]
                // Non null
          ...
          <>
            <Descriptions.Item label="Durées">
              <Space orientation="vertical">
                {props.evenement.tempsPreparation && props.evenement.tempsPreparation > 0 ? (
                  <Space>
                    <Typography.Text className="fs-09 legende">Préparation</Typography.Text>
                    <Typography.Text>{props.evenement.tempsPreparation} minutes</Typography.Text>
                  </Space>
                ) : null}
                <Space>
                  <Typography.Text className="fs-09 legende">Évènement</Typography.Text>
                  <Typography.Text>
                    {dayjs(props.evenement.fin).diff(dayjs(props.evenement.debut), "minute")}{" "}
                    minutes
                  </Typography.Text>
                </Space>
                {props.evenement.tempsSupplementaire && props.evenement.tempsSupplementaire > 0 ? (
                  <Space>
                    <Typography.Text type="secondary" className="fs-09">
                      Supplémentaire
                    </Typography.Text>
                    <Typography.Text>{props.evenement.tempsSupplementaire} minutes</Typography.Text>
                  </Space>
                ) : null}
