            ...
            {props.isGestionnaire && (
              <Tooltip title="Ouvrir dans un nouvel onglet">
                <Button
                  ...
                  onClick={(e) => {
                    e.stopPropagation();
                    window.open(`/beneficiaires/${record.beneficiaire?.uid as string}`, "_blank");
                  }}
                />
              </Tooltip>
            )}
        ...
        return (
          <Space size={0} orientation="vertical" style={{ width: 225 }}>
            <span className="light text-legende fs-09">
              {
                props.categoriesAmenagements?.find((c) => c["@id"] === typeAmenagement?.categorie)
                  ?.libelle
              }
            </span>
            <span className="semi-bold">
              {props.typesAmenagements?.find((ta) => ta["@id"] === record.typeAmenagement)?.libelle}
            </span>
          </Space>
        );
      },
